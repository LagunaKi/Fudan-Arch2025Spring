...
public:
	...
	static constexpr auto parse(addr_t addr) -> LoadOp
	{
		...
		if (Width == 8)
			return SIZE8_SHT0;
		if (Width == 4)
			return addr & 0x4 ? SIZE4_SHT32 : SIZE4_SHT0;
		if (Width == 2) {
			switch ((addr & 0x7) >> 1) {
			default:
			case 0b00: return SIZE2_SHT0;
			case 0b01: return SIZE2_SHT16;
			case 0b10: return SIZE2_SHT32;
			case 0b11: return SIZE2_SHT48;
			}
		}
		if (Width == 1) {
			switch (addr & 0x7) {
			default:
			case 0b000: return SIZE1_SHT0;
			case 0b001: return SIZE1_SHT8;
			case 0b010: return SIZE1_SHT16;
			case 0b011: return SIZE1_SHT24;
			case 0b100: return SIZE1_SHT32;
			case 0b101: return SIZE1_SHT40;
			case 0b110: return SIZE1_SHT48;
			case 0b111: return SIZE1_SHT56;
			}
		}
	}
	...
	{
		switch (value) {
		case SIZE8_SHT0:  return 0b11111111;
		case SIZE4_SHT32: return 0b11110000;
		case SIZE4_SHT0:  return 0b00001111;
		case SIZE2_SHT48: return 0b11000000;
		case SIZE2_SHT32: return 0b00110000;
		case SIZE2_SHT16: return 0b00001100;
		case SIZE2_SHT0:  return 0b00000011;
		case SIZE1_SHT56: return 0b10000000;
		case SIZE1_SHT48: return 0b01000000;
		case SIZE1_SHT40: return 0b00100000;
		case SIZE1_SHT32: return 0b00010000;
		case SIZE1_SHT24: return 0b00001000;
		case SIZE1_SHT16: return 0b00000100;
		case SIZE1_SHT8:  return 0b00000010;
		case SIZE1_SHT0:  return 0b00000001;
	...
	{
		switch (value) {
		case SIZE8_SHT0:  return 0xffffffffffffffff;
		case SIZE4_SHT32: return 0xffffffff00000000;
		case SIZE4_SHT0:  return 0x00000000ffffffff;
		case SIZE2_SHT48: return 0xffff000000000000;
		case SIZE2_SHT32: return 0x0000ffff00000000;
		case SIZE2_SHT16: return 0x00000000ffff0000;
		case SIZE2_SHT0:  return 0x000000000000ffff;
		case SIZE1_SHT56: return 0xff00000000000000;
		case SIZE1_SHT48: return 0x00ff000000000000;
		case SIZE1_SHT40: return 0x0000ff0000000000;
		case SIZE1_SHT32: return 0x000000ff00000000;
		case SIZE1_SHT24: return 0x00000000ff000000;
		case SIZE1_SHT16: return 0x0000000000ff0000;
		case SIZE1_SHT8:  return 0x000000000000ff00;
		case SIZE1_SHT0:  return 0x00000000000000ff;
	...
	constexpr void apply(word_t data, void *dest)
	{
		...
		switch (value) {
		case SIZE8_SHT0:
			*static_cast<uint64_t *>(dest) = data;
...
using CBusReqVType = uint32_t[5];  // 77 bits
using CBusRespVType = struct {
	bool ready;
	bool last;
	word_t data;
};  // 34 bits
...
struct CBusResp {
	...
	{
		return (CBusRespVType) { ready, last, data };
	}
	...
	CBusResp(const CBusRespVType &resp)
		: ready(resp.ready), last(resp.last), data(resp.data)
	{
	}
...
public:
	...
	{
		return top->cbus_req_t_addr(req) & 0xffffffff7fffffff;
	}
...
using DBusReqVType = uint32_t[5];  // 72 bits
using DBusRespVType = uint32_t[3];  // 33 bits
...
public:
	...
	auto addr() const -> addr_t
	{
		return scope->dbus_req_t_addr(ports.req) & 0xffffffff7fffffff;
	}
	auto strobe() const -> word_t
	{
		return scope->dbus_req_t_strobe(ports.req);
	}
	auto wdata() const -> word_t
	{  // req.data
	...
	{
		scope->dbus_issue_load(addr | 0x80000000, size, ports.req);
		top->eval();
	}

	void async_loadd(addr_t addr)
	{
		async_load(addr, MSIZE8);
	}
	...
	auto loadd(addr_t addr) -> word_t
	{
		return load(addr, MSIZE8);
	}

	auto loadw(addr_t addr) -> word_t
	{
		return LoadOp::parse<4>(addr).apply(load(addr, MSIZE4)) & 0xffffffff;
	}
	...
	{
		scope->dbus_issue_store(addr | 0x80000000, size, strobe, data, ports.req);
		top->eval();
	}

	void async_stored(addr_t addr, word_t data)
	{
		async_store(addr, MSIZE8, 0xff, data);
	}
	void async_storew(addr_t addr, word_t data)
	{
		auto op = LoadOp::parse<4>(addr);
		async_store(addr, MSIZE4, op.strobe(), op.place(data));
	}
	...
	void stored(addr_t addr, word_t data)
	{
		store(addr, MSIZE8, 0xff, data);
	}
	void storew(addr_t addr, word_t data)
	{
		auto op = LoadOp::parse<4>(addr);
		store(addr, MSIZE4, op.strobe(), op.place(data));
	}
...
public:
	...
	void tick()
	{
		...
		if (data_ok && valid) {
			...
			log_debug(
				"pipeline: %s \"%016lx\" @0x%x → got \"%016lx\" (size=%d, strobe=%x)\n",
	...
	void load(addr_t addr, AXISize size, void *dest, LoadOp op = LoadOp::SIZE8_SHT0)
	{
		if (!dest)
			op = LoadOp::DISCARD;
	...
	void expect(addr_t addr, AXISize size, word_t data, LoadOp op = LoadOp::SIZE8_SHT0)
	{
	...
	{
		asserts((addr & 0x7) == 0, "addr must be aligned on word boundry");
		store(addr, MSIZE8, 0b11111111u, data);
	}
	void storew(addr_t addr, word_t data)
	{
		asserts((addr & 0x3) == 0, "addr must be aligned on word boundry");
	...
	void expectd(addr_t addr, word_t data)
	{
		asserts((addr & 0x7) == 0, "addr must be aligned on word boundry");
		expect(addr, MSIZE8, data, LoadOp::SIZE8_SHT0);
	}
	...
	void expectw(addr_t addr, word_t data)
	{
		asserts((addr & 0x3) == 0, "addr must be aligned on word boundry");
		auto op = LoadOp::parse<4>(addr);
		expect(addr, MSIZE4, op.place(data), op);
	}
...
private:
	struct Task {
		...
		Task(addr_t _addr, AXISize _size, word_t _strobe, word_t _data)
			...
			dest(nullptr), load_op(LoadOp::SIZE8_SHT0)
		{
		}
		...
		void apply_assert(word_t value)
		{
			asserts(
				...
				" expected = %016lx, got = %016lx, mask = %016lx",
